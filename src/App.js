...
// ─── TÓPICOS ESTRITAMENTE DO RADAR ────────────────────────
// Cada tópico tem: bloco, titulo, p (prioridade), horas estimadas
const TOPICO_METODOLOGIA = { bloco: "Metodologia", titulo: "Aula 00 — Boas-vindas e Metodologia", p: "alta", horas: 1 };

const BASE_TOPICOS = {
  comuns: [
    // Língua Portuguesa — do Radar
...
function gerarCronograma(cargo, horasDia, progressoExistente = {}) {
  const todosTopicos = [...BASE_TOPICOS.comuns, ...(BASE_TOPICOS[cargo] || [])];
  const dias = diasAteProva();
  const totalHoras = dias * horasDia;
  const horasNecessarias = todosTopicos.reduce((s, t) => s + t.horas, 0);

  // Filtra média se tempo curto
  let lista = totalHoras < horasNecessarias * 0.6
    ? todosTopicos.filter(t => t.p !== "media")
    : todosTopicos;

  // Adiciona metodologia como primeiro tópico da lista
  const listaFinal = [TOPICO_METODOLOGIA, ...lista];

  // Agrupa por bloco mantendo ordem
  const blocoMap = {};
  const blocosOrdem = [];
  listaFinal.forEach((t, i) => {
    const id = `${cargo}_${i}`;
    if (!blocoMap[t.bloco]) {
      blocoMap[t.bloco] = { bloco: t.bloco, topicos: [] };
      blocosOrdem.push(blocoMap[t.bloco]);
    }
    blocoMap[t.bloco].topicos.push({ ...t, id, concluido: !!progressoExistente[id] });
  });

  // Distribui em semanas por horas disponíveis
...
const S = {
  ...
  container: { maxWidth: 640, margin: "0 auto", padding: "0 16px 48px" },
  ...
  card: { background: C.white, borderRadius: 12, border: `1px solid ${C.border}`, padding: "20px", marginBottom: 12 },
  ...
  btnPrimary: { width: "100%", padding: "14px", borderRadius: 10, background: C.navy, color: C.white, fontSize: 15, fontWeight: 600, border: "none", cursor: "pointer" },
  ...
  btnGreen: { width: "100%", padding: "12px", borderRadius: 10, background: C.green, color: C.white, fontSize: 15, fontWeight: 600, border: "none", cursor: "pointer" },
  ...
  semanaCard: (done) => ({ background: C.white, borderRadius: 12, border: `1px solid ${done ? C.green : C.border}`, marginBottom: 12, overflow: "hidden", opacity: done ? 0.85 : 1 }),
  semanaHeader: (done) => ({ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, background: done ? C.greenLight : C.white, borderBottom: `1px solid ${done ? "#9FE1CB" : C.border}`, cursor: "pointer" }),
  semanaNumCircle: (done) => ({ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, background: done ? C.green : C.navy, color: C.white, fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }),
  semanaTitulo: { fontSize: 14, fontWeight: 600, color: C.navy, flex: 1 },
  topicoItem: (done) => ({ display: "flex", alignItems: "flex-start", gap: 12, padding: "11px 16px", borderBottom: `1px solid ${C.silverLight}`, cursor: "pointer", background: done ? "#F8FFF9" : C.white }),
  checkbox: (done) => ({ width: 20, height: 20, borderRadius: 5, flexShrink: 0, marginTop: 1, border: done ? "none" : `2px solid ${C.border}`, background: done ? C.green : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }),
  ...
  pBadge: (p) => ({ fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 5, flexShrink: 0, marginTop: 3, background: p === "altissima" ? C.greenLight : p === "alta" ? "#E6F1FB" : C.silverLight, color: p === "altissima" ? C.greenDark : p === "alta" ? "#0C447C" : C.textSecondary }),
  ...
  alertWarn: { background: "#FFF8E6", border: "1px solid #F5C842", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "#7A5800", marginBottom: 12 },
};
  ...
  return (
    <div style={S.header}>
      ...
      <div>
        ...
        {nome && <div style={S.headerSub}>{nome}{cargo ? ` · ${CARGO_NOMES[cargo]}` : ""}</div>}
      </div>
    </div>
  );
}
...
// ─── BOX INSTALAR APP ─────────────────────────────────────
function BoxInstalarApp() {
  const [aberto, setAberto] = useState(false);
  return (
    <div style={{ ...S.card, borderLeft: `4px solid ${C.accent}`, borderRadius: "0 12px 12px 0", marginBottom: 16 }}>
      ...
      {aberto && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: C.navy, marginBottom: 6 }}>🍎 iPhone / iPad (Safari)</div>
            <ol style={{ fontSize: 13, color: C.textSecondary, lineHeight: 1.8, paddingLeft: 18 }}>
...
function TelaLogin({ onEntrar }) {
  ...
  async function handleEntrar() {
    if (!nome.trim() || !wpp.trim() || !cargo) {
      setErro("Preencha todos os campos e selecione seu cargo.");
      return;
    }
    if (wpp.replace(/\D/g, "").length < 10) {
      ...
      <div style={S.container}>
        <div style={{ padding: "24px 0 16px" }}>
          ...
          <div style={{ fontSize: 14, color: C.textSecondary, lineHeight: 1.6 }}>
            Informe seus dados para gerar ou retomar seu cronograma de estudos adaptado à sua disponibilidade.
          </div>
        </div>
        ...
        <div style={S.card}>
          ...
          <div style={S.inputGroup}>
            ...
            <input style={S.input} placeholder="Ex: 61999999999" value={wpp}
              onChange={e => setWpp(e.target.value)} inputMode="numeric" />
            <div style={{ ...S.alertWarn, marginTop: 8, marginBottom: 0, padding: "8px 12px", fontSize: 12 }}>
              ⚠️ <strong>Atenção:</strong> o WhatsApp é o identificador do seu progresso. Se digitado errado, seus dados não serão encontrados em futuros acessos. Digite exatamente como está no seu celular, com DDD.
            </div>
          </div>
          ...
          <div style={S.inputGroup}>
            ...
            <input type="range" min={1} max={8} step={1} value={horas}
              onChange={e => setHoras(Number(e.target.value))} style={{ width: "100%", accentColor: C.navy }} />
        ...
        <div style={{ textAlign: "center", fontSize: 12, color: C.textMuted, marginTop: 4 }}>
          Provas em 06/09/2026 · Inscrições até 13/07/2026 · Quadrix
        </div>
      </div>
    </>
  );
...
function TelaDashboard({ usuario, onLogout }) {
  ...
  useEffect(() => {
    const s = gerarCronograma(cargo, horasDia, progresso);
    setSemanas(s);
    if (isNovo) {
  ...
  async function toggleTopico(id) {
    ...
    setProgresso(novo);
    setSemanas(gerarCronograma(cargo, horasDia, novo));
    setSalvando(true);
    try { await apiSave({ nome, whatsapp, cargo: String(cargo), horasDia, progresso: novo }); } catch (e) {}
    setSalvando(false);
  }
  ...
  async function toggleSemana(semIdx) {
    ...
    todosIds.forEach(id => { novo[id] = !todosConcluidos; });
    setProgresso(novo);
    setSemanas(gerarCronograma(cargo, horasDia, novo));
    setSalvando(true);
    try { await apiSave({ nome, whatsapp, cargo: String(cargo), horasDia, progresso: novo }); } catch (e) {}
  ...
  async function atualizarHoras() {
    ...
    setModalHoras(false);
    setSemanas(gerarCronograma(cargo, novasHoras, progresso));
    setSalvando(true);
      ...
      <div style={S.container}>
        ...
        {showWelcome && (
          <div style={S.alertSuccess}>
            🎯 Cronograma gerado com base no Radar da Aprovação e nas suas <strong>{horasDia}h/dia</strong>. Marque os tópicos conforme for estudando — seu progresso é salvo automaticamente!
            <button onClick={() => setShowWelcome(false)} style={{ float: "right", background: "none", border: "none", cursor: "pointer", color: C.greenDark, fontSize: 18, lineHeight: 1 }}>×</button>
          </div>
        )}
        ...
        <div style={S.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12, color: C.textMuted }}>{totalConcluidos}/{totalTopicos} tópicos · {salvando ? "salvando..." : "✓ salvo"}</span>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
            ...
            <button style={S.btnSecondary} onClick={() => { setNovasHoras(horasDia); setModalHoras(true); }}>✎ Ajustar horas</button>
          </div>
        </div>
          ...
          return (
            <div key={si} style={S.semanaCard(semDone)}>
              <div style={S.semanaHeader(semDone)} onClick={() => setSemanaAberta(aberta ? -1 : si)}>
                <div style={S.semanaNumCircle(semDone)}
                  onClick={e => { e.stopPropagation(); toggleSemana(si); }}>
        ...
        <div style={{ ...S.card, borderLeft: `4px solid ${C.navy}`, borderRadius: "0 12px 12px 0" }}>
          ...
          <div style={{ fontSize: 13, color: C.textSecondary, lineHeight: 1.6 }}>
            Redação dissertativo-argumentativa (TDAS) · Mínimo 20, máximo 30 linhas · Vale 100 pts (mínimo 50) · Critérios: Conteúdo 70%, Organização 15%, Língua Portuguesa 15%.
          </div>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 20 }}>
          <div style={{ background: C.white, borderRadius: 16, padding: 24, width: "100%", maxWidth: 380 }}>
            ...
            <div style={{ fontSize: 13, color: C.textSecondary, marginBottom: 20, lineHeight: 1.5 }}>
              Os tópicos já concluídos são mantidos. O cronograma será redistribuído com os tópicos pendentes.
            </div>
            <label style={S.label}>Horas por dia: <strong style={{ color: C.navy }}>{novasHoras}h</strong></label>
            <input type="range" min={1} max={8} step={1} value={novasHoras}
              onChange={e => setNovasHoras(Number(e.target.value))} style={{ width: "100%", accentColor: C.navy, marginBottom: 20 }} />
            <div style={{ display: "flex", gap: 10 }}>
              <button style={{ ...S.btnSecondary, flex: 1 }} onClick={() => setModalHoras(false)}>Cancelar</button>
              <button style={{ ...S.btnGreen, flex: 1 }} onClick={atualizarHoras}>Atualizar cronograma</button>
            </div>
          </div>
        </div>
      )}
